...
class EquipScene {
  ...
  _renderBag(ctx) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      if (item.slots && item.slots.length > 0) {
        ...
        ctx.textAlign = 'left';
        for (var sti = 0; sti < item.slots.length; sti++) {
          var stSlot = item.slots[sti];
          if (stSlot) {
            var stDef = SlotTraitData.getDef(stSlot.id);
            if (stDef) {
              ctx.fillStyle = SlotTraitData.getColor(stSlot.id);
              ctx.fillText(stDef.label + '+' + stSlot.value, slotTextX + sti * 70, iy + 54);
            }
          }
        }
      }
    ...
    // スクロールボタン
    var maxScroll = Math.max(0, items.length * itemH - (bh - 34));
    if (maxScroll > 0) {
      var upY = by + 36;
      ctx.fillStyle = this.scrollY > 0 ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.05)';
      ctx.fillRect(bx + bw - 30, upY, 28, 22);
      ctx.font = 'bold 14px ' + CONFIG.FONT_FAMILY;
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillStyle = this.scrollY > 0 ? '#ffffff' : '#555';
      ctx.fillText('^', bx + bw - 16, upY + 11);
      var downY = by + bh - 24;
      ctx.fillStyle = this.scrollY < maxScroll ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.05)';
      ctx.fillRect(bx + bw - 30, downY, 28, 22);
      ctx.fillStyle = this.scrollY < maxScroll ? '#ffffff' : '#555';
      ctx.fillText('v', bx + bw - 16, downY + 11);
      var barH2 = Math.max(20, (bh - 34) * (bh - 34) / (items.length * itemH));
      var barY2 = by + 34 + (this.scrollY / maxScroll) * (bh - 34 - barH2);
      ctx.fillStyle = 'rgba(255,255,255,0.25)';
      ctx.fillRect(bx + bw - 4, barY2, 3, barH2);
    }
    this._bagScrollMax = maxScroll;
    this._bagListBx = bx;
    this._bagListBy = by;
    this._bagListBw = bw;
    this._bagListBh = bh;
  ...
  onTap(x, y) {
    ...
    // バッグスクロール
    if (this._bagScrollMax > 0 && this._bagListBx) {
      var sbx = this._bagListBx + this._bagListBw - 30;
      var sby2 = this._bagListBy + 36;
      var sdby = this._bagListBy + this._bagListBh - 24;
      if (x >= sbx && x <= sbx + 28 && y >= sby2 && y <= sby2 + 22) {
        this.scrollY = Math.max(0, this.scrollY - 120);
        return;
      }
      if (x >= sbx && x <= sbx + 28 && y >= sdby && y <= sdby + 22) {
        this.scrollY = Math.min(this._bagScrollMax, this.scrollY + 120);
        return;
      }
    }
